...
export default function CarGrid({
  ...
  const openVehicle = (event: React.MouseEvent<HTMLAnchorElement>, carId: string) => {
    ...
    if (openingCarId === carId) return;
    setOpeningCarId(carId);
    setLocation(`/fleet/${carId}`);
  };
          ...
          <div className="min-w-0">
            <div className="flex flex-col gap-3 md:flex-row md:items-center">
              <div className="relative min-w-0 flex-1">
                ...
                <input value={searchTerm} onChange={(event) => onSearchChange(event.target.value)} placeholder="Search a model or class" className="h-11 w-full rounded-[10px] border border-[#E8E0D5] bg-white pl-10 pr-4 text-sm font-medium text-[#0f1e2e] outline-none transition placeholder:text-sm placeholder:font-normal placeholder:text-slate-400 focus:border-[#0f1e2e] focus:ring-4 focus:ring-[#f3eee8]" aria-label="Search the fleet" />
              </div>
              <label className="relative flex h-10 w-full items-center gap-2 rounded-[10px] border border-[#E8E0D5] bg-white px-3 text-[13px] font-medium text-slate-500 md:w-[220px]">
                <SlidersHorizontal className="h-3.5 w-3.5 text-slate-500" />
                <span className="sr-only">Sort the fleet</span>
                <select aria-label="Sort the fleet" className="w-full appearance-none bg-transparent pr-2 text-[13px] font-medium text-[#0f1e2e] outline-none" value={sortBy} onChange={(event) => onSortChange(event.target.value as SortBy)}>
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3"><p className="text-[13px] font-medium text-slate-500">Showing <strong className="font-semibold text-[#0f1e2e]">{cars.length}</strong> of {totalCars} demo vehicles</p>{filterChips.length > 0 && <div className="flex flex-wrap items-center gap-2">{filterChips.map((chip) => <span key={chip} className="rounded-full bg-[#eaf0f3] px-3 py-1.5 text-[11px] font-extrabold text-slate-600">{chip}</span>)}<button onClick={onClearFilters} className="text-[11px] font-extrabold text-[#b45309] transition hover:text-[#0f1e2e]">Clear all</button></div>}</div>
...
function FilterSidebar({ selectedCategory, priceBand, passengerBand, fuelFilter, hasActiveFilters, onCategoryChange, onPriceBandChange, onPassengerBandChange, onFuelFilterChange, onClear }: { selectedCategory: "All" | CarType; priceBand: PriceBand; passengerBand: PassengerBand; fuelFilter: FuelFilter; hasActiveFilters: boolean; onCategoryChange: (value: "All" | CarType) => void; onPriceBandChange: (value: PriceBand) => void; onPassengerBandChange: (value: PassengerBand) => void; onFuelFilterChange: (value: FuelFilter) => void; onClear: () => void }) {
  return <aside aria-label="Fleet filters" className="rounded-xl border border-[#E8E0D5] bg-white p-4 shadow-[0_4px_16px_rgba(67,49,34,0.04)]"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2.5"><span className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#f3eee8] text-[#0f1e2e]"><SlidersHorizontal className="h-3.5 w-3.5" /></span><div><p className="text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-400">Fleet filters</p><h3 className="mt-0.5 text-[17px] font-semibold leading-none tracking-[-0.02em] text-[#0f1e2e]">Refine the route</h3></div></div><button disabled={!hasActiveFilters} onClick={onClear} className="flex h-7 items-center gap-1.5 rounded-lg px-2 text-xs font-medium text-slate-500 transition hover:bg-[#f7f4f0] hover:text-[#0f1e2e] disabled:cursor-not-allowed disabled:opacity-40"><RotateCcw className="h-3.5 w-3.5" /> Reset</button></div><div className="mt-2.5 grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><FilterGroup title="Vehicle type">{fleetCategories.map((category) => <FilterChoice key={category} checked={selectedCategory === category} label={category === "All" ? "All vehicle types" : category} onClick={() => onCategoryChange(category)} />)}</FilterGroup><FilterGroup title="Fuel type">{(["all", "petrol", "hybrid", "electric"] as FuelFilter[]).map((fuel) => <FilterChoice key={fuel} checked={fuelFilter === fuel} label={fuel === "all" ? "Any fuel" : `${fuel[0].toUpperCase()}${fuel.slice(1)}`} onClick={() => onFuelFilterChange(fuel)} />)}</FilterGroup><FilterGroup title="Daily rate">{priceFilters.map((filter) => <FilterChoice key={filter.value} checked={priceBand === filter.value} label={filter.label} onClick={() => onPriceBandChange(filter.value)} />)}</FilterGroup><FilterGroup title="Passenger capacity">{passengerFilters.map((filter) => <FilterChoice key={filter.value} checked={passengerBand === filter.value} label={filter.label} onClick={() => onPassengerBandChange(filter.value)} />)}</FilterGroup></div></aside>;
}

function FilterGroup({ title, children }: { title: string; children: React.ReactNode }) { return <section className="rounded-xl border border-[#E8E0D5] bg-[#fdfbf9] p-2.5"><p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-slate-400">{title}</p><div className="mt-1.5 flex flex-wrap gap-1.5">{children}</div></section>; }
function FilterChoice({ checked, label, onClick }: { checked: boolean; label: string; onClick: () => void }) { return <button onClick={onClick} aria-pressed={checked} className={`inline-flex h-7 w-auto items-center gap-1.5 rounded-lg px-2.5 py-1 text-left text-xs font-medium leading-none transition ${checked ? "bg-[#0f1e2e] text-white" : "border border-[#E8E0D5] bg-white text-slate-600 hover:border-slate-400 hover:text-[#0f1e2e]"}`}><span className={`flex h-3 w-3 shrink-0 items-center justify-center rounded-[3px] border ${checked ? "border-white/70 bg-white/10 text-white" : "border-slate-300 bg-white"}`}>{checked && <Check className="h-2.5 w-2.5" />}</span>{label}</button>; }
...
function LeadVehicle({ car, saved, onBook, onToggleSaved, onOpenDetails, reduceMotion }: { car: FleetCar; saved: boolean; onBook: (car: FleetCar) => void; onToggleSaved: (carId: string) => void; onOpenDetails: (event: React.MouseEvent<HTMLAnchorElement>, carId: string) => void; reduceMotion: boolean }) {
  return <motion.article layout initial={{ opacity: 0, y: reduceMotion ? 0 : 12 }} whileInView={{ opacity: 1, y: 0 }} whileHover={reduceMotion ? undefined : { y: -4 }} viewport={{ once: true, amount: 0.18 }} exit={{ opacity: 0, y: reduceMotion ? 0 : 12 }} transition={{ duration: 0.26, ease: [0.23, 1, 0.32, 1] }} className="fleet-card group overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-[0_4px_24px_rgba(0,0,0,0.08)] transition duration-300 hover:border-slate-400 hover:shadow-[0_25px_58px_rgba(15,30,46,0.13)] lg:grid lg:grid-cols-[0.9fr_1.1fr]">
    <Link href={`/fleet/${car.id}`} onClick={(event) => onOpenDetails(event, car.id)} className="relative block min-h-[250px] overflow-hidden bg-[#eef2f3] p-3 lg:min-h-[360px] lg:p-4" aria-label={`View full details for ${car.name}`}><img src={car.image} alt={`${car.name} ${car.modelYear}`} className="h-full w-full object-contain object-center transition duration-500 group-hover:scale-[1.015]" /><div className="absolute inset-0 bg-gradient-to-t from-[#0f1e2e]/35 via-transparent to-transparent transition group-hover:from-[#0f1e2e]/45" /><div className="absolute bottom-5 left-5 flex items-end gap-4 text-white"><span className="font-editorial text-5xl tracking-[-0.07em]">01</span><span className="fleet-kicker mb-1.5 text-white/75">Lead selection / {car.type}</span></div></Link>
    <div className="flex flex-col p-5 sm:p-6 lg:px-7 lg:pb-6 lg:pt-7"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><p className="fleet-kicker text-slate-400">Featured for today</p><Link href={`/fleet/${car.id}`} onClick={(event) => onOpenDetails(event, car.id)} className="mt-3 block max-w-[26rem] font-editorial text-[40px] leading-[0.95] tracking-[-0.055em] text-[#0f1e2e] transition hover:text-[#d97706] sm:text-[44px]">{car.name}</Link><p className="mt-2 text-sm font-bold text-slate-500">{car.modelYear} · {car.accent}</p></div><div className="flex shrink-0 items-center gap-2"><span className="rounded-full bg-orange-50 px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-[0.1em] text-[#b45309]">Available</span><SaveButton saved={saved} carId={car.id} onToggleSaved={onToggleSaved} /></div></div><p className="mt-4 max-w-md border-l-2 border-[#d97706] pl-4 text-sm leading-6 text-slate-600">{car.note}</p><div className="my-5 grid grid-cols-[1fr_1.35fr_1fr] border-y border-slate-200"><SpecRail label="Energy" value={car.fuel} /><SpecRail label="Transmission" value={car.transmission} /><SpecRail label="Capacity" value={`${car.seats} seats`} /></div><div className="mt-auto flex flex-wrap items-end justify-between gap-5"><div><p className="fleet-kicker text-slate-400">Daily rate</p><p className="mt-1 font-editorial text-5xl tracking-[-0.065em] text-[#d97706]">${car.rate}<span className="ml-1 font-sans text-sm font-bold tracking-normal text-slate-400">/ day</span></p></div><div className="flex flex-wrap gap-3"><Link href={`/fleet/${car.id}`} onClick={(event) => onOpenDetails(event, car.id)} className="fleet-action flex h-11 items-center gap-2 rounded-xl border-[1.5px] border-slate-300 px-4 text-sm font-extrabold text-[#0f1e2e] transition hover:border-[#0f1e2e]">Full details <ChevronRight className="h-4 w-4" /></Link><button onClick={() => onBook(car)} className="fleet-action flex h-11 items-center gap-2 rounded-xl bg-[#d97706] px-5 text-sm font-extrabold text-white transition hover:bg-[#b45309] active:scale-[0.97]">Reserve <ArrowUpRight className="h-4 w-4" /></button></div></div></div>
  </motion.article>;
}
...
function ComparisonVehicle({ car, index, saved, onBook, onToggleSaved, onOpenDetails, reduceMotion }: { car: FleetCar; index: number; saved: boolean; onBook: (car: FleetCar) => void; onToggleSaved: (carId: string) => void; onOpenDetails: (event: React.MouseEvent<HTMLAnchorElement>, carId: string) => void; reduceMotion: boolean }) {
  return <motion.article layout initial={{ opacity: 0, y: reduceMotion ? 0 : 14 }} whileInView={{ opacity: 1, y: 0 }} whileHover={reduceMotion ? undefined : { y: -5 }} viewport={{ once: true, amount: 0.12 }} exit={{ opacity: 0, y: reduceMotion ? 0 : 12 }} transition={{ duration: 0.26, delay: reduceMotion ? 0 : index * 0.06, ease: [0.23, 1, 0.32, 1] }} className="fleet-card group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_4px_24px_rgba(0,0,0,0.08)] transition duration-300 hover:border-slate-400 hover:shadow-[0_22px_42px_rgba(15,30,46,0.12)]">
    <Link href={`/fleet/${car.id}`} onClick={(event) => onOpenDetails(event, car.id)} className="relative block h-48 overflow-hidden bg-[#eef2f3] p-2 sm:h-52 sm:p-3" aria-label={`View full details for ${car.name}`}><img src={car.image} alt={`${car.name} ${car.modelYear}`} className="h-full w-full object-contain object-center transition duration-500 group-hover:scale-[1.015]" /><div className="absolute inset-0 bg-gradient-to-t from-[#0f1e2e]/12 to-transparent opacity-0 transition duration-300 group-hover:opacity-100" /><span className="fleet-kicker absolute left-3 top-3 rounded-full bg-white/95 px-2.5 py-1 text-[#0f1e2e]"><span className="h-1.5 w-1.5 rounded-full bg-[#16a34a]" /> {car.type}</span><span className="absolute inset-0 flex items-center justify-center"><span className="translate-y-2 rounded-xl bg-white px-4 py-2 text-xs font-extrabold text-[#0f1e2e] opacity-0 shadow-lg transition duration-200 group-hover:translate-y-0 group-hover:opacity-100">Open details</span></span><span className="fleet-kicker absolute bottom-3 left-3 rounded-full bg-white/95 px-2.5 py-1 text-[#0f1e2e]">0{index + 2} / {car.badge}</span></Link>
    <span className="absolute right-3 top-3"><SaveButton saved={saved} carId={car.id} onToggleSaved={onToggleSaved} /></span>
    <div className="p-4 sm:p-5"><div className="flex flex-col items-start gap-1"><div className="min-w-0"><Link href={`/fleet/${car.id}`} onClick={(event) => onOpenDetails(event, car.id)} className="block font-editorial text-[21px] font-semibold leading-[1.05] tracking-[-0.04em] text-[#0f1e2e] transition hover:text-[#d97706]">{car.name}</Link><p className="mt-1 text-xs font-bold text-slate-500">{car.modelYear}</p></div><p className="flex items-baseline font-editorial text-[21px] font-bold tracking-[-0.045em] text-[#0f1e2e]">${car.rate}<span className="ml-0.5 font-sans text-sm font-bold tracking-normal text-[#0f1e2e]">/day</span></p></div><p className="fleet-card-description mt-2 text-[13px] leading-[1.5] text-[#666]">{car.accent}</p><div className="mt-3 grid grid-cols-3 border-y border-slate-100"><SpecRail label="Energy" value={car.fuel} compact /><SpecRail label="Drive" value={car.transmission} compact /><SpecRail label="Cabin" value={`${car.seats} seats`} compact /></div><div className="mt-3 flex items-center justify-between gap-3"><Link href={`/fleet/${car.id}`} onClick={(event) => onOpenDetails(event, car.id)} className="fleet-action flex items-center gap-1.5 text-[13px] font-extrabold text-[#0f1e2e] transition hover:text-[#d97706]">Vehicle profile <ChevronRight className="h-3.5 w-3.5" /></Link><button onClick={() => onBook(car)} className="fleet-action text-[13px] font-extrabold uppercase tracking-[0.08em] text-[#b45309]">Reserve</button></div></div>
  </motion.article>;
}
