...
function LeadVehicle({ car, saved, onBook, onToggleSaved, onOpenDetails, reduceMotion }: { car: FleetCar; saved: boolean; onBook: (car: FleetCar) => void; onToggleSaved: (carId: string) => void; onOpenDetails: (event: React.MouseEvent<HTMLAnchorElement>, carId: string) => void; reduceMotion: boolean }) {
  return <motion.article layout initial={{ opacity: 0, y: reduceMotion ? 0 : 12 }} whileInView={{ opacity: 1, y: 0 }} whileHover={reduceMotion ? undefined : { y: -4 }} viewport={{ once: true, amount: 0.18 }} exit={{ opacity: 0, y: reduceMotion ? 0 : 12 }} transition={{ duration: 0.26, ease: [0.23, 1, 0.32, 1] }} className="fleet-card group overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-[0_4px_24px_rgba(0,0,0,0.08)] transition duration-300 hover:border-slate-400 hover:shadow-[0_25px_58px_rgba(15,30,46,0.13)] lg:grid lg:grid-cols-[0.9fr_1.1fr]">
    <Link href={`/fleet/${car.id}`} onClick={(event) => onOpenDetails(event, car.id)} className="relative block min-h-[250px] overflow-hidden bg-[#eef2f3] lg:min-h-[360px]" aria-label={`View full details for ${car.name}`}><img src={car.image} alt={`${car.name} ${car.modelYear}`} className="absolute inset-0 h-full w-full object-cover object-center transition duration-500 group-hover:scale-[1.025]" /><div className="absolute inset-0 bg-gradient-to-t from-[#0f1e2e]/50 via-transparent to-transparent transition group-hover:from-[#0f1e2e]/60" /><div className="absolute bottom-5 left-5 flex items-end gap-4 text-white"><span className="font-editorial text-5xl tracking-[-0.07em]">01</span><span className="fleet-kicker mb-1.5 text-white/75">Lead selection / {car.type}</span></div></Link>
...
function ComparisonVehicle({ car, index, saved, onBook, onToggleSaved, onOpenDetails, reduceMotion }: { car: FleetCar; index: number; saved: boolean; onBook: (car: FleetCar) => void; onToggleSaved: (carId: string) => void; onOpenDetails: (event: React.MouseEvent<HTMLAnchorElement>, carId: string) => void; reduceMotion: boolean }) {
  return <motion.article layout initial={{ opacity: 0, y: reduceMotion ? 0 : 14 }} whileInView={{ opacity: 1, y: 0 }} whileHover={reduceMotion ? undefined : { y: -5 }} viewport={{ once: true, amount: 0.12 }} exit={{ opacity: 0, y: reduceMotion ? 0 : 12 }} transition={{ duration: 0.26, delay: reduceMotion ? 0 : index * 0.06, ease: [0.23, 1, 0.32, 1] }} className="fleet-card group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_4px_24px_rgba(0,0,0,0.08)] transition duration-300 hover:border-slate-400 hover:shadow-[0_22px_42px_rgba(15,30,46,0.12)]">
    <Link href={`/fleet/${car.id}`} onClick={(event) => onOpenDetails(event, car.id)} className="relative block h-48 overflow-hidden bg-[#eef2f3] sm:h-52" aria-label={`View full details for ${car.name}`}><img src={car.image} alt={`${car.name} ${car.modelYear}`} className="h-full w-full object-cover object-center transition duration-500 group-hover:scale-[1.025]" /><div className="absolute inset-0 bg-gradient-to-t from-[#0f1e2e]/18 to-transparent opacity-0 transition duration-300 group-hover:opacity-100" /><span className="fleet-kicker absolute left-3 top-3 rounded-full bg-white/95 px-2.5 py-1 text-[#0f1e2e]"><span className="h-1.5 w-1.5 rounded-full bg-[#16a34a]" /> {car.type}</span><span className="absolute inset-0 flex items-center justify-center"><span className="translate-y-2 rounded-xl bg-white px-4 py-2 text-xs font-extrabold text-[#0f1e2e] opacity-0 shadow-lg transition duration-200 group-hover:translate-y-0 group-hover:opacity-100">Open details</span></span><span className="fleet-kicker absolute bottom-3 left-3 rounded-full bg-white/95 px-2.5 py-1 text-[#0f1e2e]">0{index + 2} / {car.badge}</span></Link>
