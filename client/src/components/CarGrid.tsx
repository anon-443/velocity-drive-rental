...
function FilterSidebar({ selectedCategory, priceBand, passengerBand, fuelFilter, hasActiveFilters, onCategoryChange, onPriceBandChange, onPassengerBandChange, onFuelFilterChange, onClear }: { selectedCategory: "All" | CarType; priceBand: PriceBand; passengerBand: PassengerBand; fuelFilter: FuelFilter; hasActiveFilters: boolean; onCategoryChange: (value: "All" | CarType) => void; onPriceBandChange: (value: PriceBand) => void; onPassengerBandChange: (value: PassengerBand) => void; onFuelFilterChange: (value: FuelFilter) => void; onClear: () => void }) {
  return <aside aria-label="Fleet filters" className="filter-bar-dense rounded-[18px] border border-slate-200 bg-white p-3 sm:p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2.5"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#eaf0f3] text-[#0f1e2e]"><SlidersHorizontal className="h-3.5 w-3.5" /></span><div><p className="fleet-kicker text-slate-400">Fleet filters</p><h3 className="mt-0.5 font-editorial text-[21px] leading-none tracking-[-0.045em] text-[#0f1e2e]">Refine the route</h3></div></div><button disabled={!hasActiveFilters} onClick={onClear} className="flex h-8 items-center justify-center gap-1.5 rounded-lg border border-slate-200 px-2.5 text-[11px] font-extrabold text-slate-600 transition hover:border-[#0f1e2e] hover:text-[#0f1e2e] disabled:cursor-not-allowed disabled:opacity-40"><RotateCcw className="h-3.5 w-3.5" /> Reset</button></div><div className="filter-groups-dense mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-4"><FilterGroup title="Vehicle type">{fleetCategories.map((category) => <FilterChoice key={category} checked={selectedCategory === category} label={category === "All" ? "All vehicle types" : category} onClick={() => onCategoryChange(category)} />)}</FilterGroup><FilterGroup title="Fuel type">{(["all", "petrol", "hybrid", "electric"] as FuelFilter[]).map((fuel) => <FilterChoice key={fuel} checked={fuelFilter === fuel} label={fuel === "all" ? "Any fuel" : `${fuel[0].toUpperCase()}${fuel.slice(1)}`} onClick={() => onFuelFilterChange(fuel)} />)}</FilterGroup><FilterGroup title="Daily rate">{priceFilters.map((filter) => <FilterChoice key={filter.value} checked={priceBand === filter.value} label={filter.label} onClick={() => onPriceBandChange(filter.value)} />)}</FilterGroup><FilterGroup title="Passenger capacity">{passengerFilters.map((filter) => <FilterChoice key={filter.value} checked={passengerBand === filter.value} label={filter.label} onClick={() => onPassengerBandChange(filter.value)} />)}</FilterGroup></div></aside>;
}

function FilterGroup({ title, children }: { title: string; children: React.ReactNode }) { return <section className="filter-group-dense rounded-xl border border-slate-100 bg-[#fdfcfb] px-2.5 py-2"><p className="fleet-kicker text-slate-400">{title}</p><div className="filter-options-dense mt-1.5 grid grid-cols-2 gap-1">{children}</div></section>; }
function FilterChoice({ checked, label, onClick }: { checked: boolean; label: string; onClick: () => void }) { return <button onClick={onClick} aria-pressed={checked} className={`filter-choice-dense flex min-h-7 w-full items-center gap-1.5 rounded-md px-1.5 py-1 text-left text-[10px] font-bold leading-tight transition sm:text-[11px] ${checked ? "bg-[#eaf0f3] text-[#0f1e2e]" : "text-slate-500 hover:bg-slate-50 hover:text-[#0f1e2e]"}`}><span className={`flex h-3 w-3 shrink-0 items-center justify-center rounded-[3px] border ${checked ? "border-[#0f1e2e] bg-[#0f1e2e] text-white" : "border-slate-300 bg-white"}`}>{checked && <Check className="h-2 w-2" />}</span><span className="min-w-0 truncate">{label}</span></button>; }
