...
function FilterSidebar({ selectedCategory, priceBand, passengerBand, fuelFilter, hasActiveFilters, onCategoryChange, onPriceBandChange, onPassengerBandChange, onFuelFilterChange, onClear }: { selectedCategory: "All" | CarType; priceBand: PriceBand; passengerBand: PassengerBand; fuelFilter: FuelFilter; hasActiveFilters: boolean; onCategoryChange: (value: "All" | CarType) => void; onPriceBandChange: (value: PriceBand) => void; onPassengerBandChange: (value: PassengerBand) => void; onFuelFilterChange: (value: FuelFilter) => void; onClear: () => void }) {
  return <aside aria-label="Fleet filters" className="rounded-xl border border-[#E8E0D5] bg-white p-5 shadow-[0_4px_16px_rgba(67,49,34,0.04)]"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2.5"><span className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#f3eee8] text-[#0f1e2e]"><SlidersHorizontal className="h-3.5 w-3.5" /></span><div><p className="text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-400">Fleet filters</p><h3 className="mt-0.5 text-[18px] font-semibold leading-none tracking-[-0.02em] text-[#0f1e2e]">Refine the route</h3></div></div><button disabled={!hasActiveFilters} onClick={onClear} className="flex h-8 items-center gap-1.5 rounded-lg px-2 text-xs font-medium text-slate-500 transition hover:bg-[#f7f4f0] hover:text-[#0f1e2e] disabled:cursor-not-allowed disabled:opacity-40"><RotateCcw className="h-3.5 w-3.5" /> Reset</button></div><div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><FilterGroup title="Vehicle type">{fleetCategories.map((category) => <FilterChoice key={category} checked={selectedCategory === category} label={category === "All" ? "All vehicle types" : category} onClick={() => onCategoryChange(category)} />)}</FilterGroup><FilterGroup title="Fuel type">{(["all", "petrol", "hybrid", "electric"] as FuelFilter[]).map((fuel) => <FilterChoice key={fuel} checked={fuelFilter === fuel} label={fuel === "all" ? "Any fuel" : `${fuel[0].toUpperCase()}${fuel.slice(1)}`} onClick={() => onFuelFilterChange(fuel)} />)}</FilterGroup><FilterGroup title="Daily rate">{priceFilters.map((filter) => <FilterChoice key={filter.value} checked={priceBand === filter.value} label={filter.label} onClick={() => onPriceBandChange(filter.value)} />)}</FilterGroup><FilterGroup title="Passenger capacity">{passengerFilters.map((filter) => <FilterChoice key={filter.value} checked={passengerBand === filter.value} label={filter.label} onClick={() => onPassengerBandChange(filter.value)} />)}</FilterGroup></div></aside>;
}

function FilterGroup({ title, children }: { title: string; children: React.ReactNode }) { return <section className="rounded-xl border border-[#E8E0D5] bg-[#fdfbf9] p-3"><p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-slate-400">{title}</p><div className="mt-2 flex flex-wrap gap-2">{children}</div></section>; }
function FilterChoice({ checked, label, onClick }: { checked: boolean; label: string; onClick: () => void }) { return <button onClick={onClick} aria-pressed={checked} className={`inline-flex h-8 w-auto items-center gap-1.5 rounded-lg px-3.5 py-1.5 text-left text-[13px] font-medium leading-none transition ${checked ? "bg-[#0f1e2e] text-white" : "border border-[#E8E0D5] bg-white text-slate-600 hover:border-slate-400 hover:text-[#0f1e2e]"}`}><span className={`flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded-[3px] border ${checked ? "border-white/70 bg-white/10 text-white" : "border-slate-300 bg-white"}`}>{checked && <Check className="h-2.5 w-2.5" />}</span>{label}</button>; }
