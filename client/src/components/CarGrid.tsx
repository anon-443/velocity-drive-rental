...
function FilterSidebar({ selectedCategory, priceBand, passengerBand, fuelFilter, hasActiveFilters, onCategoryChange, onPriceBandChange, onPassengerBandChange, onFuelFilterChange, onClear }: { selectedCategory: "All" | CarType; priceBand: PriceBand; passengerBand: PassengerBand; fuelFilter: FuelFilter; hasActiveFilters: boolean; onCategoryChange: (value: "All" | CarType) => void; onPriceBandChange: (value: PriceBand) => void; onPassengerBandChange: (value: PassengerBand) => void; onFuelFilterChange: (value: FuelFilter) => void; onClear: () => void }) {
  return <aside className="filter-rail-compact h-fit rounded-[18px] border border-slate-200 bg-white p-3.5 xl:sticky xl:top-24"><div className="flex items-start justify-between gap-3"><div><p className="eyebrow"><span /> Filter rail</p><h3 className="mt-2 font-editorial text-[26px] tracking-[-0.055em] text-[#0f1e2e]">Narrow the route</h3></div><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#eaf0f3] text-[#0f1e2e]"><SlidersHorizontal className="h-3.5 w-3.5" /></span></div><FilterGroup title="Vehicle type">{fleetCategories.map((category) => <FilterChoice key={category} checked={selectedCategory === category} label={category === "All" ? "All vehicle types" : category} onClick={() => onCategoryChange(category)} />)}</FilterGroup><FilterGroup title="Fuel type">{(["all", "petrol", "hybrid", "electric"] as FuelFilter[]).map((fuel) => <FilterChoice key={fuel} checked={fuelFilter === fuel} label={fuel === "all" ? "Any fuel" : `${fuel[0].toUpperCase()}${fuel.slice(1)}`} onClick={() => onFuelFilterChange(fuel)} />)}</FilterGroup><FilterGroup title="Daily rate">{priceFilters.map((filter) => <FilterChoice key={filter.value} checked={priceBand === filter.value} label={filter.label} onClick={() => onPriceBandChange(filter.value)} />)}</FilterGroup><FilterGroup title="Passenger capacity">{passengerFilters.map((filter) => <FilterChoice key={filter.value} checked={passengerBand === filter.value} label={filter.label} onClick={() => onPassengerBandChange(filter.value)} />)}</FilterGroup><button disabled={!hasActiveFilters} onClick={onClear} className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg border border-slate-200 py-2.5 text-xs font-extrabold text-slate-600 transition hover:border-[#0f1e2e] hover:text-[#0f1e2e] disabled:cursor-not-allowed disabled:opacity-40"><RotateCcw className="h-3.5 w-3.5" /> Reset filters</button></aside>;
}

function FilterGroup({ title, children }: { title: string; children: React.ReactNode }) { return <div className="filter-group-compact mt-4 border-t border-slate-100 pt-3"><p className="fleet-kicker text-slate-400">{title}</p><div className="filter-options-compact mt-2 grid gap-1">{children}</div></div>; }
function FilterChoice({ checked, label, onClick }: { checked: boolean; label: string; onClick: () => void }) { return <button onClick={onClick} aria-pressed={checked} className={`filter-choice-compact flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-[11px] font-bold transition ${checked ? "bg-[#eaf0f3] text-[#0f1e2e]" : "text-slate-500 hover:bg-slate-50 hover:text-[#0f1e2e]"}`}><span className={`flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded border ${checked ? "border-[#0f1e2e] bg-[#0f1e2e] text-white" : "border-slate-300 bg-white"}`}>{checked && <Check className="h-2.5 w-2.5" />}</span>{label}</button>; }
