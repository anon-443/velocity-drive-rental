...
export default function Home() {
  ...
  const scrollToFleet = () => { const target = document.querySelector<HTMLElement>("#fleet-start"); if (!target) return; const headerHeight = document.querySelector<HTMLElement>(".interior-nav")?.offsetHeight ?? 76; const top = Math.max(0, target.getBoundingClientRect().top + window.scrollY - headerHeight); window.scrollTo({ top, behavior: "smooth" }); };
  ...
  return <div id="home" className="velocity-interior min-h-screen overflow-x-hidden bg-[#f7f8f6] text-[#0f1e2e]"><Navbar /><main>
    ...
    <div id="fleet-start" aria-hidden="true" className="h-0 scroll-mt-20" />
